...
export default function TrainerClientLogsPage() {
  ...
  return (
    <PageContainer>
      <div className="flex items-center gap-2 mb-2">
        <Link href="/trainer/client/sam" className="text-muted-foreground hover:text-foreground font-mono-ui text-[10px] uppercase tracking-wider">
          ← Sam&apos;s Dashboard
        </Link>
      </div>
      ...
      <PageHeader 
        title="Training Logs" 
        subtitle="Chronological history for Sam"
      />

      <SurfaceCard className="mb-4">
        <div className="font-mono-ui text-[10px] uppercase tracking-[0.18em] text-muted-foreground mb-4">Weekly Engagement</div>
              ...
              <tbody>
                {logs.map((l, i) => (
                  <tr key={i} className="border-b border-border last:border-0 hover:bg-surface-2 transition-colors">
                    ...
                    <td className="py-2.5 px-2 text-[11px] text-muted-foreground max-w-[150px] truncate" title={l.notes || ""}>
                      {l.notes || ""}
                    </td>
                  </tr>
                ))}
